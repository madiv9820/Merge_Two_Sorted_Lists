...
def _make_testcase(testcase):
    ...
    def test_method(self):
        ...
        list1           :   List[int]   = testcase['input']['list1']
        list2           :   List[int]   = testcase['input']['list2']
        ...
        try:
            list1_head: Optional[ListNode] = _create_linked_list(values=list1)
            list2_head: Optional[ListNode] = _create_linked_list(values=list2)
            actual_output: List[int] = self._run_case(list1=list1_head, list2=list2_head)
        except TimeoutError:
...
class TestSolution(unittest.TestCase):
    ...
    @timeout(1)
    def _run_case(self, list1: Optional[ListNode], list2: Optional[ListNode]) -> List[int]:
        # ⚡ Give every testcase its own timeout instead of timing the whole file.
        head: Optional[ListNode] = self.__solution.mergeTwoLists(list1=list1, list2=list2)
        result: List[int] = []
...
class MergeTwoListsTestResult(unittest.TextTestResult):
    ...
    def stopTestRun(self):
        ...
        print('\n' + '🧾' + '=' * 58)
        print('📚 Merge Two Sorted Lists Final Summary')
...
if __name__ == '__main__':
    unittest.main(
        verbosity=2,
        testRunner=unittest.TextTestRunner(resultclass=MergeTwoListsTestResult)
    )
